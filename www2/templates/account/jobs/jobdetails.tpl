...
<tr><td align="center">
	{if $nbitems neq 0}
        ...
        <br>
        <b>STATUS</b>: Term: <b>{$n_term}</b> / Run: <b>{$n_run}</b> / RemoteWait: <b>{$n_rwait}</b> / Wait: <b>{$n_wait}</b> / Errors: <b>{if $n_err > 0}<font color="#FF0000">{/if}{$n_err}{if $n_err > 0}</font>{/if}</b> / resubmissions: <b>{$resubmissions}</b>%
        <p>
