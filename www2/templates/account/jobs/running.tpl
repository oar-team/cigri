...
<tr><td align="center">
	...
	{if $nbitems neq 0}
		Running MultiJobs {$minindex} - {$maxindex} out of {$nbitems}
		{* parity var *}
		...
		{foreach from=$eventarray item=secondkey}
			...
			{/if}
			<tr class="{$trclass}">
				<td align="center"><a href="account.php?submenu=jobs&option=runningdetails&id={$secondkey[0]}">{$secondkey[0]}</a></td>
