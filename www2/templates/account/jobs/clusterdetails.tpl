...
<tr><td align="center">
	...
	{if $nbitems neq 0}
		Cluster {$name} - MultiJobs {$minindex} - {$maxindex} out of {$nbitems}
		{* parity var *}
		...
		<tr class="titlerow">
			...
			<th><a href="{$itemsorderby[2]}">State{$itemsorderimgs[2]}</a></th>
		</tr>
		{foreach from=$eventarray item=secondkey}
			...
			{/if}
			<tr class="{$trclass}">
				<td align="center"><a href="account.php?submenu=jobs&option=clusterdistribution&name={$queryname}&multijobid={$secondkey[0]}">{$secondkey[0]}</a></td>
