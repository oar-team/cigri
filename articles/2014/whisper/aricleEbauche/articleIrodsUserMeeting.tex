...
\\
The iRODS storage infrastructure of Ciment consists of a unique zone with the iCat server and a dozen of nodes as illustrated on figure \ref{schemaCimentStorageNodesIcat}. The nodes are groupped inside 3 different locations, called site A, site B and site C, having heterogeneous WAN connexions. Each site has it's own 10Gbe local network switch. Those 3 sites are located into the 3 main datacenters where the CIMENT supercomuters live, so there are always close storage resources with the computing nodes. Note that site C has only a 1Gbe WAN connexion while A and B have 10Gbe WAN connexions.
