...
# Size of the window in seconds on wich the job throughput 
# is calculated
...
# Calculate the job throughput (in jobs/seconds) in the time window
def job_troughput(jobs,time_window_size,last_terminated)
  nb=0
  first_submited=Time.now.to_i
  jobs.each do |job|
    job_tsub = to_unix_time(job['jobTSub'])
    if job['jobState'] == 'Terminated' && job_tsub > (Time.now-time_window_size).to_i
      nb+=1
      first_submited = job_tsub if job_tsub < first_submited
    end	
  end
  return nb.to_f / (last_terminated - first_submited).to_f
end

def forecast(dbh,mjob_id,time_window_size)
	...
	jobs.each do |job|
			case job['jobState'] 
				...
				when 'Running', 'toLaunch', 'RemoteWaiting'
					nb_waiting += 1
			end		
	end
	...
	troughput=job_troughput(jobs,time_window_size,last_terminated)
	forcasted = (nb_waiting / troughput).to_i

	res = { 'mjob_id' => mjob_id, 'forcaster' => 'troughput', 'duration' => 0, 'end_time' => 0, 
	...
	if (nb == 0) 
		...
		troughput=(3600*nb/total).to_f
		res['data'] =  {'jobs_per_hour' => sprintf("%.2f",troughput).to_f,\
		                'average' => sprintf("%.2f",mean).to_f , \
				'standard_deviation' => sprintf("%.2f",std_dev).to_f }
		res['duration'] = total
		...
		res['data'] =  {'jobs_per_hour' => sprintf("%.2f",troughput*3600).to_f, \
		                'average' => sprintf("%.2f",mean).to_f , \
				'standard_deviation' => sprintf("%.2f",std_dev).to_f } 
...
if (ARGV.length != 0)
	#forecast(dbh, 'MJobsId' => ARGV[0].to_i,time_window_size)
	forecast(dbh, ARGV[0].to_i,time_window_size)
else
	get_all_multiplejobs(dbh).each do |mjob|
		forecast(dbh,mjob,time_window_size)
	end
end
