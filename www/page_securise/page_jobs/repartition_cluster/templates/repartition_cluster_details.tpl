{include file="header.tpl" title1= "repartition of the clusters (details)" title2= "CLUSTER REPARTITION"}

	votre login: {$login}
	<a href = "repartition_cluster.php">  <p align="right">return</p></a>
	</br>
	<CENTER><h1>  MULTIJOBS ON THE CLUSTER  "{$clustername}"</h1></CENTER>
	</br>
	...
	<table border bordercolor="#9999CC" cellspacing="1">
		...
		<tr>
			...
			<td>
				<select name="cleprimaire">
					...
					{if $cleprimaire == "jobMJobsId"} <option value="jobMJobsId" selected > {else}<option value="jobMJobsId"  > {/if}Job Id
					{if $cleprimaire == "MJobsName"} <option value="MJobsName" selected > {else}<option value="MJobsName"  > {/if}Job Name
		...
		<tr>
			...
			<td>
				...
				{if  $clesecondaire== "jobMJobsId"} <option value="jobMJobsId" selected > {else}<option value="jobMJobsId"  > {/if}Job Id
				{if $clesecondaire == "MJobsName"} <option value="MJobsName" selected > {else}<option value="MJobsName"  > {/if}Job Name
...
-->
	 ...
	 the number of jobs on the  cluster {$clustername}: {$nb_total}
	</br>
	</br>
	{if $nb_ligne != 0 }
		<table border="1" cellspacing="0" cellpadding="0">
			...
			{section name=i loop=$reponse}
				<tr>
					<td>{$reponse[i].jobMJobsId}</td>
					<td>{$reponse[i].MJobsState }</td>
					<td>
						<form method="post" action="repartition_cluster_details_parametres.php">
 							<input type="submit" name="bidule" value="parameters">
 							<input type="hidden" name="clustername" value="{$clustername}">
							<input type="hidden" name="ID" value="{$reponse[i].jobMJobsId}">
 						</form>
					</td>
...
{if $nb_total > 100}
			 ...
			 {section name=i loop=$pages}
 				...
 				<a Href= "repartition_cluster_details.php?ID={$ID}&page_courante={$pages[i]}&nb_jobs={$nb_jobs}&cleprimaire={$cleprimaire}&clesecondaire={$clesecondaire}&sensprim={$sensprim}&senssecond={$senssecond}&clustername={$clustername}&clic=1">
				{if $page_courante == $pages[i]}
