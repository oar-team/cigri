...
$params = array(
    array('file',""),array('b',0),array('c',0),array('s',1),array('sat',0)
    );
...
function AdjSat($img,$sat) {
	...
	echo "AdjSat: Colors total: $nbr<br>";
	for( $i=0; $i<$nbr; ++$i ) {
...
function AdjBrightContrast($img,$bright,$contr) {
	$nbr = imagecolorstotal ($img);
	echo "AdjBrightCOnstrast: Colors total: $nbr<br>";
	for( $i=0; $i<$nbr; ++$i ) {
		$colarr = imagecolorsforindex ($img,$i);
...
imagecopyresampled($img,$bkg,0,0,0,0,$w,$h,$bw,$bh);
