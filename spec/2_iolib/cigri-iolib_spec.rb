...
describe 'cigri-iolib' do
  ...
  %w{Pg}.each do |db_type|
    describe "#{db_type}" do
      ...
      describe 'db_connect' do
        ...
        #  lambda{db_connect()}.should raise_error RDBI::Error
        #end
      end # db_connect
    end # db_type
  end # %w{}.each
  ...
  describe 'get_cluster_id' do
    it 'should return an ID if the cluster exists' do
      db_connect() do |dbh|
        get_cluster_id(dbh, 'dahu').should be_a(Integer)
      end
    end
  ...
  describe 'get_clusters_ids' do
    ...
    it 'should return a hash ID if the clusters exists' do
      db_connect() do |dbh|
        res = get_clusters_ids(dbh, %w{dahu bigfoot non_existing.cluster})
  ...
  describe 'cigri_submit' do
    it 'should submit a campaign' do
      db_connect() do |dbh|
        ...
        id = cigri_submit(dbh, @correct_json, 'bzizou')
  ...
  describe "cigri_submit_jobs" do
    ...
    it 'should fail if campaign is cancelled' do
      db_connect() do |dbh|
        ...
        dbh.execute("UPDATE campaigns SET state = 'cancelled' WHERE id = ?", id)
    ...
    it 'should change a campaign from terminated to in_treatment' do
      db_connect() do |dbh|
        nb_params = @correct_json['params'].length
        id = cigri_submit(dbh, @correct_json, 'kameleon')
        dbh.execute("UPDATE campaigns SET state = 'terminated' WHERE id = ?", id)
        lambda{cigri_submit_jobs(dbh, ["param1 a", "param2 b"], id, 'kameleon')}.should_not raise_error
        dbh.execute("SELECT state FROM campaigns WHERE id = ?", id).fetch(:first)[0].should be == "in_treatment"
        dbh.execute("SELECT count(*) FROM bag_of_tasks WHERE campaign_id = ?", id).fetch(:first)[0].should be == nb_params + 2
        delete_campaign(dbh, 'kameleon', id)
      end
    end
